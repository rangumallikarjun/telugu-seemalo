...
// ── Customize / custom-order request modal ───────────────────────────────────
function CustomizeModal({ product, user, sz, clr, price, shareUrl, storePhone, onClose }) {
  const [name, setName] = useState(user?.name || "");
  const [details, setDetails] = useState("");

  const waDigits = (storePhone || "").replace(/\D/g, "");
  ...
  const send = () => {
    ...
    const lines = [
      ...
      price ? `Listed price: ${fmt(price)}` : null,
      sz ? `Size: ${sz}` : null,
      clr?.name ? `Colour: ${clr.name}` : null,
      name.trim() ? `Customer: ${name.trim()}` : null,
    ...
    const url = waDigits ? `https://wa.me/${waDigits}?text=${text}` : `https://wa.me/?text=${text}`;
  ...
  return (
    <div className="modal-bg" onClick={onClose}>
      <div className="modal" style={{ maxWidth: 460 }} onClick={e => e.stopPropagation()}>
        ...
        <p>Tell us what you need — custom size, colour, quantity, personalised text or any special request. We'll continue on WhatsApp.</p>
        ...
        {user && (
          <div style={{ fontSize: ".85rem", color: "var(--mt)", marginBottom: 14 }}>
        ...
        <div className="inp-grp">
          ...
          <textarea
            ...
            rows={5}
            placeholder={"e.g. Size 24×36 inches\nColour: deep maroon background\nAdd name 'Lakshmi & Ram' at the bottom\nQuantity: 2"}
            style={{ width:"100%", padding:"11px 14px", border:"1.5px solid var(--bd)", borderRadius:8, fontSize:".95rem", fontFamily:"'DM Sans',sans-serif", outline:"none", resize:"vertical" }}
          />
        </div>

        <button className="modal-btn" onClick={send} disabled={!details.trim()}
          style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:8, opacity: details.trim() ? 1 : .55 }}>
      ...
      <div className="pd-wrap">
        ...
        <div className="pd-grid">
          ...
          <div className="pd-info">
            ...
            {p.customizable && (
              <button className="pd-customize-btn" onClick={() => setCustomOpen(true)}>
